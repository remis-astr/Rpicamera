...
class RPiCameraLiveStackAdvanced:
    ...
    def process_frame(self, image_data: np.ndarray) -> Optional[np.ndarray]:
        ...
        if self.use_lucky and self.lucky_stacker:
            ...
            self.stats['lucky_stacks_done'] = lucky_stats.get('stacks_done', 0)
            self.stats['accepted_frames'] = lucky_stats.get('frames_selected', 0)

            # Vérifier si un nouveau stack Lucky est disponible
            result = self.lucky_stacker.get_result()
            if result is not None:
                # MODE CUMULATIF : Envoyer le stack Lucky vers l'advanced_stacker
                # pour l'accumuler avec les cycles précédents
                if self.use_advanced_stacking and self.advanced_stacker:
                    # Ajouter le stack Lucky au master stack
                    self.advanced_stacker.add_image(result, quality_metrics={})
                    self.stats['accepted_frames'] = self.advanced_stacker.count
